-- build_analytics_snapshot_v1_1
--
-- Analytics Snapshot v1.1 semantic cleanup. Adds a NEW versioned snapshot
-- builder alongside (never replacing) the v1.0 functions in
-- 20260728000000_build_analytics_snapshot_v1.sql. v1.0's functions and
-- every previously stored v1.0 analytics_runs.snapshot row remain
-- unchanged and fully readable — this migration only ADDS new objects.
--
-- ── WHAT CHANGED FROM v1.0 (see analytics/SEMANTIC_CONTRACT.md section 7.1
-- for the full rationale) ───────────────────────────────────────────────
-- 1. acquisition_value_status: a new derived field (positive / zero_assigned
--    / unknown / negative_invalid) exposed wherever item-level or cohort
--    logic needs it. zero_assigned (acquisition_value = 0, a possibly
--    intentional assigned value) is NEVER treated as unknown
--    (acquisition_value IS NULL).
-- 2. Acquisition Value Band labels: the old combined "Zero / unknown" label
--    is split into "Zero assigned value" and "Unknown acquisition value"
--    (plus a defensive "Negative (invalid)" catch-all for data-quality
--    rows). The six POSITIVE bands ($1-999 ... $5,000+) are unchanged.
-- 3. New special-value summaries: zero_assigned_value_summary and
--    unknown_acquisition_value_summary in the acquisition_value_band
--    module, so zero/unknown items are reported rather than silently
--    excluded now that they no longer appear in positive_value_performance.
-- 4. Exit-count naming: total_realized_sale_exit_count /
--    total_realized_trade_exit_count (every realized exit of that type) are
--    now explicitly distinct from eligible_transition_sale_exit_count /
--    eligible_transition_trade_exit_count (positive-value-transition-
--    eligible only), with explicit zero/unknown exclusion counts
--    reconciling the two.
-- 5. Holding-day naming: raw_realized_holding_days_present_count (merely
--    non-null) vs. eligible_realized_holding_days_count (realized,
--    non-historical, non-null, no lifecycle date issue) are now distinct,
--    and every holding_sample_size in every module uses the SAME
--    eligibility rule (v1.0 was missing the lifecycle-date-issue check in
--    several queries — see analytics/sql/01_acquisition_value_band_
--    performance.sql's Query B/C/F1/G1/G3/G4 for the corrected reference).
-- 6. Brand-count naming: all_business_distinct_brand_count (every Business
--    item) vs. positive_acquisition_distinct_brand_count (positive
--    acquisition value only) vs. decision_ready_distinct_brand_count
--    (brands passing the decision-ready sample-size threshold) are three
--    explicitly distinct populations, never sharing one field name.
-- 7. Estimated-upside coverage: a known ZERO acquisition value now
--    correctly contributes $0 to a capital SUM (v1.0's `FILTER (WHERE
--    acquisition_value > 0)` silently produced NULL for an all-zero-
--    acquisition band/brand instead of 0 — fixed to `FILTER (WHERE
--    acquisition_value IS NOT NULL)`). Ambiguous estimated_upside_missing_
...
-- 8. Recommendation candidates gain acquisition_value_status,
--    acquisition_value_band_label, and estimated_upside_status alongside
--    the existing estimated_net_upside.
--
-- No existing business calculation for POSITIVE acquisition values changes
-- — positive_value_performance (renamed from v1.0's `performance` key) is
-- the same query, same population, same numbers as v1.0's `performance`.
--
-- ── STRUCTURE ────────────────────────────────────────────────────────────
-- One top-level callable function:
--   public.build_analytics_snapshot_v1_1(p_recommendation_target_user_id int)
--     RETURNS jsonb
-- backed by four private helpers, exactly mirroring v1.0's own structure:
--   public._build_acquisition_value_band_snapshot_v1_1()
--   public._build_acquisition_to_exit_snapshot_v1_1()
--   public._build_brand_snapshot_v1_1()
--   public._build_recommendation_candidates_snapshot_v1_1(p_recommendation_target_user_id int)
--
-- ── SECURITY MODEL — IDENTICAL TO v1.0 ───────────────────────────────────
...
-- target is always an explicit int argument, validated against
-- public.app_users before use. EXECUTE is revoked from PUBLIC/anon/
-- authenticated on every function — service_role only, same rationale as
-- v1.0 (see that migration's own SECURITY MODEL section).
-- ============================================================================


-- ============================================================================
-- 1. public._build_acquisition_value_band_snapshot_v1_1()
-- Reproduces analytics/sql/01_acquisition_value_band_performance.sql (v1.1).
-- Included sections mapped to stable JSON keys:
--   Query A1       -> population_summary
--   Query A2       -> band_coverage_counts
--   Query B        -> positive_value_performance (renamed from v1.0's `performance`)
--   Query B-ZERO   -> zero_assigned_value_summary (NEW in v1.1)
--   Query B-UNKNOWN-> unknown_acquisition_value_summary (NEW in v1.1)
...
-- EXCLUDED: Query G2 (developer-only per-user diagnostic) and Query G5
-- (developer-only item-level drilldown) — same exclusions as v1.0.
-- ============================================================================

CREATE OR REPLACE FUNCTION public._build_acquisition_value_band_snapshot_v1_1()
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY INVOKER
...
a1_row AS (
  SELECT
    ...
    (SELECT MIN(acquisition_value) FROM business WHERE acquisition_value_status = 'positive')               AS min_acquisition_value_positive,
    (SELECT MAX(acquisition_value) FROM business WHERE acquisition_value_status = 'positive')               AS max_acquisition_value_positive,
    (SELECT ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY acquisition_value)::numeric, 2)
       FROM business WHERE acquisition_value_status = 'positive')                                           AS median_acquisition_value_positive,
...
-- Query B -> positive_value_performance (v1.0's `performance`, unchanged
-- population/numbers, only the holding_sample_size eligibility fix applied)
b_rows AS (
  SELECT
...
-- Query D1 -> capital_efficiency (unchanged from v1.0 — already had the
-- lifecycle-date-issue check in its timing_eligible population)
d1_capital AS (
  SELECT
    acquisition_value_band_order,
    acquisition_value_band_label,
...
-- Query D2 -> capital_efficiency_sensitivity (unchanged from v1.0)
d2_primary AS (
...
-- Query E1 -> open_listed_inventory (v1.1 FIX: known-zero sums to 0, not
-- NULL; negative_invalid excluded; new coverage fields)
e1_listed_items AS (
...
e1_rows AS (
  SELECT
    ...
    lf.acquisition_value_known_count,
    lf.acquisition_value_zero_assigned_count,
    lf.acquisition_value_unknown_count,
    lf.estimated_value_missing_count,
    lf.estimated_upside_available_count,
    lf.estimated_upside_indeterminate_count,
...
-- Query E2 -> open_unlisted_inventory (same v1.1 fixes as E1)
...
e2_rows AS (
  SELECT
    ...
    uf.acquisition_value_known_count,
    uf.acquisition_value_zero_assigned_count,
    uf.acquisition_value_unknown_count,
    uf.estimated_value_missing_count,
    uf.estimated_upside_available_count,
    uf.estimated_upside_indeterminate_count,
...
SELECT jsonb_build_object(
  ...
  'positive_value_performance',      (SELECT COALESCE(jsonb_agg(to_jsonb(b_rows) ORDER BY acquisition_value_band_order), '[]'::jsonb) FROM b_rows),
  'zero_assigned_value_summary',     (SELECT COALESCE(jsonb_agg(to_jsonb(bzero_row)), '[]'::jsonb) FROM bzero_row),
  'unknown_acquisition_value_summary',(SELECT COALESCE(jsonb_agg(to_jsonb(bunknown_row)), '[]'::jsonb) FROM bunknown_row),
...
REVOKE ALL ON FUNCTION public._build_acquisition_value_band_snapshot_v1_1() FROM PUBLIC;
REVOKE ALL ON FUNCTION public._build_acquisition_value_band_snapshot_v1_1() FROM anon;
REVOKE ALL ON FUNCTION public._build_acquisition_value_band_snapshot_v1_1() FROM authenticated;


-- ============================================================================
-- 2. public._build_acquisition_to_exit_snapshot_v1_1()
-- Reproduces analytics/sql/02_acquisition_to_exit_analysis.sql (v1.1). This
-- module computes NO holding_days-derived metric (unchanged from v1.0 —
-- "Acquisition-to-Exit" means value movement, not duration). Included
-- sections mapped to stable JSON keys:
--   Query A1 -> population_summary (v1.1: reworked exit-count naming)
...
--
-- v1.1: Sections B/C/C2/D/E/F/G's own band CASE blocks are UNCHANGED from
-- v1.0 (still using the combined "Zero / unknown" label) — this file's
-- `eligible` population already requires a positive acquisition value AND
-- a positive exit value, so that branch is provably unreachable in every
-- one of these sections. Only Query A1, which reports the REALIZED
-- population BEFORE that restriction, needed the v1.1 rework. See
-- analytics/sql/02_acquisition_to_exit_analysis.sql's own header for the
-- same scope decision.
-- ============================================================================

CREATE OR REPLACE FUNCTION public._build_acquisition_to_exit_snapshot_v1_1()
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY INVOKER
...
REVOKE ALL ON FUNCTION public._build_acquisition_to_exit_snapshot_v1_1() FROM PUBLIC;
REVOKE ALL ON FUNCTION public._build_acquisition_to_exit_snapshot_v1_1() FROM anon;
REVOKE ALL ON FUNCTION public._build_acquisition_to_exit_snapshot_v1_1() FROM authenticated;


-- ============================================================================
-- 3. public._build_brand_snapshot_v1_1()
-- Reproduces analytics/sql/03_brand_performance.sql (v1.1). Included
-- sections mapped to stable JSON keys:
--   Query A1 -> population_summary (v1.1: brand-count naming reworked)
...
--   Query E1 -> open_listed_inventory (v1.1: estimated-upside coverage reworked)
--   Query E2 -> open_unlisted_inventory (v1.1: estimated-upside coverage reworked)
--   Query F  -> cohort_comparison
--   Query I  -> capital_concentration
--   Query H2 -> integrity_summary (v1.1: brand-count naming reworked)
-- EXCLUDED: Query G (developer-only per-user diagnostic) and Query H
-- (developer-only item-level drilldown).
-- ============================================================================

CREATE OR REPLACE FUNCTION public._build_brand_snapshot_v1_1()
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY INVOKER
...
a1_row AS (
  SELECT
    (SELECT COUNT(*) FROM acquisition_value_band)                                                                       AS total_lifecycle_rows,
    (SELECT COUNT(*) FROM business)                                                                          AS business_items,
    (SELECT COUNT(*) FROM business WHERE acquisition_value_status = 'positive')                              AS business_positive_acquisition_items,
    (SELECT COUNT(*) FROM business WHERE acquisition_value_status = 'zero_assigned')                         AS business_zero_assigned_acquisition_items,
    (SELECT COUNT(*) FROM business WHERE acquisition_value_status = 'unknown')                                AS business_unknown_acquisition_items,
    (SELECT COUNT(*) FROM business WHERE acquisition_value_status = 'negative_invalid')                       AS business_negative_acquisition_items,
    (SELECT COUNT(*) FROM business WHERE acquisition_value_status <> 'positive')                              AS business_zero_or_unknown_acquisition_items,
    (SELECT COUNT(*) FROM business WHERE is_realized)                                                        AS realized_business_items,
    (SELECT COUNT(*) FROM business WHERE NOT is_realized)                                                    AS open_business_items,
    (SELECT COUNT(DISTINCT brand_label) FROM business)                                                       AS all_business_distinct_brand_count,
    (SELECT COUNT(*) FROM (
       SELECT brand_label FROM business
       WHERE acquisition_value_status = 'positive'
       GROUP BY brand_label
       HAVING COUNT(*) >= 3 AND COUNT(*) FILTER (WHERE is_realized) >= 3
     ) decision_ready_brands)                                                                                 AS decision_ready_distinct_brand_count,
...
f_combined AS (
  SELECT 'All eligible Business items' AS population_label,
    brand_label, is_realized, net_profit, roi, holding_days, global_days_on_market, is_historical_import, has_lifecycle_date_issue
  FROM eligible
  UNION ALL
  SELECT 'Imported historical Business items' AS population_label,
    brand_label, is_realized, net_profit, roi, holding_days, global_days_on_market, is_historical_import, has_lifecycle_date_issue
...
h2_row AS (
  SELECT
    COUNT(*)                                                                                   AS positive_acquisition_item_count,
    COUNT(DISTINCT brand_label)                                                                 AS positive_acquisition_distinct_brand_count,
...
REVOKE ALL ON FUNCTION public._build_brand_snapshot_v1_1() FROM PUBLIC;
REVOKE ALL ON FUNCTION public._build_brand_snapshot_v1_1() FROM anon;
REVOKE ALL ON FUNCTION public._build_brand_snapshot_v1_1() FROM authenticated;


-- ============================================================================
-- 4. public._build_recommendation_candidates_snapshot_v1_1(p_recommendation_target_user_id int)
-- recommendation_candidates.open_business_items. Still a FACTUAL CANDIDATE
-- DATASET ONLY — no recommendation type (reprice/list/relist/trade
-- out/bundle), score, ranking, or reason code is produced here.
--
-- Eligibility: analytics_item_lifecycle.user_id = p_recommendation_target_user_id
-- AND purpose_name = 'Business' AND NOT is_realized — IDENTICAL to v1.0.
--
-- v1.1 additions (Part 10): acquisition_value_status, acquisition_value_
-- band_label, and estimated_upside_status alongside the existing
-- estimated_net_upside. Rules:
--   positive acquisition value  -> normal calculation, status 'available'
--     (or 'missing_estimated_value' if estimated_sold_value is unknown).
--   zero assigned acquisition value -> estimated_net_upside still valid
--     (upside = estimated_sold_value - 0 - expenses); ROI would be
--     undefined but ROI is not part of this candidate row.
--   unknown acquisition value -> estimated_net_upside is NULL by
--     construction (the CASE below requires acquisition_value IS NOT
--     NULL); status 'unknown_acquisition_value'.
--   negative_invalid -> integrity/problem state; status
--     'other_indeterminate', no normal economics implied.
-- ============================================================================

CREATE OR REPLACE FUNCTION public._build_recommendation_candidates_snapshot_v1_1(
...
rows AS (
  SELECT
    ...
    acquisition_method,
    acquisition_value,
    acquisition_value_status,
    CASE
      WHEN acquisition_value_status = 'unknown'          THEN 'Unknown acquisition value'
      WHEN acquisition_value_status = 'zero_assigned'    THEN 'Zero assigned value'
      WHEN acquisition_value_status = 'negative_invalid' THEN 'Negative (invalid)'
      WHEN acquisition_value < 1000 THEN '$1-999'
      WHEN acquisition_value < 2000 THEN '$1,000-1,999'
      WHEN acquisition_value < 3000 THEN '$2,000-2,999'
      WHEN acquisition_value < 4000 THEN '$3,000-3,999'
    ...
    CASE
      WHEN estimated_sold_value IS NULL THEN 'missing_estimated_value'
      WHEN acquisition_value_status = 'unknown' THEN 'unknown_acquisition_value'
      WHEN acquisition_value_status = 'negative_invalid' THEN 'other_indeterminate'
      ELSE 'available'
    END AS estimated_upside_status,
    first_listed_at,
...
REVOKE ALL ON FUNCTION public._build_recommendation_candidates_snapshot_v1_1(int) FROM PUBLIC;
REVOKE ALL ON FUNCTION public._build_recommendation_candidates_snapshot_v1_1(int) FROM anon;
REVOKE ALL ON FUNCTION public._build_recommendation_candidates_snapshot_v1_1(int) FROM authenticated;


-- ============================================================================
-- 5. public.build_analytics_snapshot_v1_1(p_recommendation_target_user_id int)
-- Top-level callable function. Validates the target exactly like v1.0's
-- build_analytics_snapshot_v1, then assembles the v1.1 snapshot contract:
--
-- {
...
--   "evidence_aggregates": {
--     "acquisition_value_band": { ... 16 keys, see function 1 above ... },
...
-- }
--
-- Top-level shape is otherwise IDENTICAL to v1.0's — only the two version
-- strings and the inner field names/sections described above change.
-- requested_by_user_id, run status, and error_message remain analytics_runs
-- metadata, not part of this contract.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.build_analytics_snapshot_v1_1(
  p_recommendation_target_user_id int
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  IF p_recommendation_target_user_id IS NULL THEN
    RAISE EXCEPTION 'build_analytics_snapshot_v1_1: p_recommendation_target_user_id must not be NULL';
  END IF;
  ...
  IF v_target_count <> 1 THEN
    RAISE EXCEPTION 'build_analytics_snapshot_v1_1: expected exactly 1 app_users row for id % (recommendation target), found %',
  ...
  RETURN jsonb_build_object(
    ...
    'evidence_aggregates', jsonb_build_object(
      'acquisition_value_band', public._build_acquisition_value_band_snapshot_v1_1(),
...
REVOKE ALL ON FUNCTION public.build_analytics_snapshot_v1_1(int) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.build_analytics_snapshot_v1_1(int) FROM anon;
REVOKE ALL ON FUNCTION public.build_analytics_snapshot_v1_1(int) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.build_analytics_snapshot_v1_1(int) TO service_role;

GRANT EXECUTE ON FUNCTION public._build_acquisition_value_band_snapshot_v1_1() TO service_role;
GRANT EXECUTE ON FUNCTION public._build_acquisition_to_exit_snapshot_v1_1() TO service_role;
GRANT EXECUTE ON FUNCTION public._build_brand_snapshot_v1_1() TO service_role;
GRANT EXECUTE ON FUNCTION public._build_recommendation_candidates_snapshot_v1_1(int) TO service_role;

COMMENT ON FUNCTION public.build_analytics_snapshot_v1_1(int) IS
  'Analytics Snapshot v1.1 semantic cleanup builder. SECURITY INVOKER, '
  'service_role execution only. Same security model and evidence/'
  'recommendation boundary as build_analytics_snapshot_v1 (v1.0), which '
  'remains unchanged and callable — this is a new, additive version, not a '
  'replacement. See analytics/SEMANTIC_CONTRACT.md section 7.1 and '
  'analytics/README.md for the full v1.1 changelog.';
