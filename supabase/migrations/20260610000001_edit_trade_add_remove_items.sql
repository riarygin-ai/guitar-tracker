-- Fix edit_trade_operation: revert ALL current outgoing items to 'owned' before
-- re-processing, not just the ones in the new list.
--
-- Old step 3 only reverted items still in the new p_outgoing_items array.
-- If an outgoing item was removed from the trade its status stayed 'traded'.
-- New step 3 reads the current deal_items rows (before they are deleted in step 5)
-- and reverts every outgoing item.  Step 7 then re-marks the kept ones as 'traded'.
...
BEGIN
  ...
  -- 8. Re-insert incoming deal_items (inventory status stays 'owned')
