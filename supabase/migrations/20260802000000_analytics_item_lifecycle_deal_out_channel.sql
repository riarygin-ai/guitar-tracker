-- analytics_item_lifecycle: add Deal Out Channel columns
--
-- Channel Analytics, module 2 of N: Deal Out Channel Performance (see
...
-- which an item LEFT inventory (Marketplace / Kijiji / Reverb / Regular
-- Buyer / Seller). It is NOT a payment method, NOT a shipping method, and
-- NOT "the technical place where the deal was completed." For a cash sale,
-- this is the sale deal's deal_channel_id. For a trade, the existing
-- deal_channel_id on that ONE trade deal is the one counterparty/
-- contact-source channel for the whole deal — outgoing and incoming items
-- on the same trade deal are never assigned separate channels. Open
-- (not-yet-realized) items have deal_out_channel_id = NULL, always — there
-- is no exit deal yet to read a channel from.
--
-- ── WHY NEW COLUMNS INSTEAD OF REUSING exit_channel_id/_name ────────────
-- The view already exposes exit_channel_id/exit_channel_name (added in
-- 20260723000000_analytics_item_lifecycle.sql), sourced from exactly the
-- same exit-deal join this migration reads from. Those names predate this
-- project's "no ambiguous analytics names" naming rule
...
-- forward, mirroring deal_in_channel_id/_name/_requires_listing (added in
-- 20260731000000_analytics_item_lifecycle_deal_in_channel.sql); exit_channel_id/
-- _name remain as they were, unchanged.
--
...
--   deal_out_channel_id               bigint  — same value as
--     exit_channel_id (dc.id from the exit deal's channel). NULL for open
--     items (no exit_deal row at all) and for a realized trade whose
--     deal_channel_id was left unset (create_trade_operation's p_channel_id
--     is optional, unlike create_sell_operation's, which is required) — a
--     real, visible "missing channel" state, never hidden or defaulted.
--   deal_out_channel_name             text    — same value as
--     exit_channel_name (dc.name).
--   deal_out_channel_requires_listing boolean — dc.is_listing_platform for
--     that channel; NULL when deal_out_channel_id is NULL.
--
-- Everything else — SECURITY INVOKER behavior, user_id, every existing
-- column, every existing calculation, including all Deal In Channel columns
-- added in 20260731000000 — is UNCHANGED. This migration is a pure
-- additive column extension.
-- ============================================================================
...
exit_deal AS (
  SELECT DISTINCT ON (do_.item_id)
    ...
    -- Deal Out Channel (new) — same join, explicit Channel-Analytics naming.
...
SELECT
  ...
  -- ── Deal Out Channel (new) — appended at the very end so every prior
  -- column's position is preserved for CREATE OR REPLACE VIEW. ───────────
...
COMMENT ON VIEW public.analytics_item_lifecycle IS
  ...
  'acquisition-deal channel already exposed as acquisition_channel_id/_name. deal_out_channel_id/_name/'
  '_requires_listing (added 20260802000000) are the Channel-Analytics-facing names for the SAME exit-deal '
  'channel already exposed as exit_channel_id/_name — NULL for open items (no exit deal yet) and for a '
  'realized trade whose channel was left unset, never defaulted to a fake value. See '
  'analytics/sql/04_deal_in_channel_performance.sql, analytics/sql/05_deal_out_channel_performance.sql, and '
  'analytics/SEMANTIC_CONTRACT.md for the full Deal In/Out Channel definitions. See migration file header '
  'for full notes on listing-exposure limitations, Historical Import placeholder-date limitations, and the '
