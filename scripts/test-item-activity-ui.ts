...
console.log('\n[D — /leads item_attributed semantics]');
{
  ...
  check('contextual "← Listings" link in the /leads header', /href="\/listings"/.test(pageNc) && /←/.test(page) && /Listings\s*<\/Link>/.test(pageNc) && /action=\{/.test(pageNc));
...
console.log('\n[E — /listings: section placement, window, fetching]');
{
  ...
  check('item data fetched only for the CURRENT trend window (matches trend_window_weeks, not loading)', /demandEvidence\.trend_window_weeks === trendWeeks/.test(page) && /!demandLoading/.test(page));
  check('one compact request per window, keyed on the window (never per item / per row)', /\[itemWindowKey\]/.test(page) && (page.match(/fetchListingItemActivity\(/g) ?? []).length === 1 && !/\/api\/leads/.test(page));
  check('stale windows are never shown (result key must equal current window key)', /itemActivity\.key === itemWindowKey/.test(page));
