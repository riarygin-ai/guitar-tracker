-- ============================================================================
-- 12_open_inventory_decision_support_v2.sql
--
-- Business questions:
--   - For one target user, what does their OPEN inventory look like across
--     every Purpose — Business, Hybrid, Personal, missing-Purpose, and
--     missing-policy?
--   - Which open Business items deserve urgency attention (unlisted, slow
--     DOM relative to comparable listed items, long ownership age)?
--   - Which open Hybrid items behave more like active Business inventory,
--     and which behave more like long-term Personal inventory — with
--     enough neutral evidence for the USER to decide, not the analytics
--     layer?
--   - Which open Personal items concentrate capital, lack an estimate, or
--     otherwise need portfolio-visibility attention — WITHOUT treating
--     long ownership or slow DOM as inherently negative?
--
...
-- already joined into that view). Nothing in this file creates a database
-- object and nothing here writes to production data. See
-- analytics/README.md and analytics/SEMANTIC_CONTRACT.md section 24.
--
...
-- redesign are all OUT of scope for this file.
--
-- ── THIS MODULE NEVER PRODUCES A SCORE, RECOMMENDED ACTION, OR AUTOMATIC
--    PURPOSE CHANGE ──────────────────────────────────────────────────────
-- No `score`, `priority_score`, `recommended_action`, `recommended_
-- purpose`, `reclassify_to_business`, `reclassify_to_personal`, or
-- `keep_hybrid` value exists anywhere below. `reason_codes` /
...
-- a single number. This file never writes to inventory_items.purpose_id.
--
-- ── HYBRID IS NOT ENCODED AS "BAD" ───────────────────────────────────────
-- Hybrid items are commercially relevant with personal interest or
-- appreciation potential — longer holding may be intentional. They get
-- NEITHER Business-style urgency NOR Personal-style "no urgency"
-- treatment, but their own neutral REVIEW evidence (the user's own stated
-- goal is to periodically decide whether each Hybrid item truly belongs
-- in Business or Personal — this file exposes evidence for that decision,
-- it never makes the decision).
--
-- ── TWO SEPARATE COHORT OBJECTS ──────────────────────────────────────────
-- `economic_cohort` (median profit/ROI/realization rate) pools ALL
-- Purposes — economic eligibility is shared regardless of Purpose.
-- Hierarchy (7 levels, no exact-model level — `model` is free text with
-- no normalization table, see MODEL_COHORT_UNAVAILABLE_FREE_TEXT_MODEL_
-- FIELD): brand+type+band -> brand+band -> brand -> category+band ->
-- category -> band -> all inventory.
-- `liquidity_cohort` (DOM/holding time/realization speed) PREFERS the
-- item's own current Purpose first (4 levels: brand+type+band+purpose ->
-- brand+band+purpose -> category+band+purpose -> purpose alone), then
-- falls back to 4 cross-purpose levels (reusing the same underlying
-- brand+type+band / brand+band / category+band / all-inventory pools the
-- economic cohort uses) if no purpose-matched cohort clears the standard
-- confidence bar. `liquidity_cohort_match` is 'purpose_matched' /
-- 'cross_purpose_fallback' / 'unavailable'; a cross-purpose fallback adds
-- the reason code PURPOSE_MATCHED_LIQUIDITY_COHORT_UNAVAILABLE. Both
-- cohorts use the SAME selection rule as every other module in this
-- analytics layer: first candidate (specificity order) with
-- realized_item_count >= 5, else first with >= 3, else the most specific
-- with >= 1, else no cohort.
--
-- ── PURPOSE-AWARE URGENCY ─────────────────────────────────────────────────
-- Business: DOM urgency only at current_dom_days >= 30; ownership-age
-- urgency at 120+ reliable days; unlisted open items may carry
-- BUSINESS_UNLISTED_OPEN_ITEM.
-- Hybrid: no Business-style urgency at all. HYBRID_REVIEW_REQUIRED is
-- unconditional on every Hybrid row. Listed/unlisted/long-hold/high-
-- capital/low-upside/recent-history are REVIEW signals, never a
-- realization recommendation.
-- Personal: NO DOM or ownership-age urgency code exists for Personal.
-- PERSONAL_AGE_UNRELIABLE is a data-completeness flag, never urgency.
-- Focus is capital concentration, missing estimates, zero/unknown values,
-- and a neutral listed-for-opportunistic-exit note — long ownership and
-- slow DOM are never treated as negative.
-- Items with missing/unmapped Purpose ('unclassified') receive only the
-- GENERAL evidence codes — no urgency is assumed for an unclassified
-- disposition.
--
-- ── QUERY CLASSIFICATION INDEX ───────────────────────────────────────────
-- Query A/B/C/D/E/F below are all TARGET-USER-ONLY evidence — restricted
-- to one user_id (see the `= 2` filter — REPLACE 2 with a real user id).
-- economic_cohort/liquidity_cohort STATISTICS pool every user's items
-- (all Purposes), but no other user's item_id, item name, model, or any
-- other item-identity field is ever exposed, and no shared/cohort-level
-- output is grouped by user.
--
-- Queries A-F share ONE CTE pipeline (all_items -> level_* / liq_level_*
-- cohort pools -> target_base -> target_facts) and are assembled together
-- below into a single jsonb object, exactly matching
-- public._build_open_inventory_decision_support_snapshot_v2(int)'s own
-- shape (supabase/migrations/20260812000000_build_analytics_snapshot_v2_1.sql).
...
--   Query F -> personal_inventory_control.personal_position_summary
-- ============================================================================
...
target_base AS (
  ...
  FROM all_items ai
  WHERE ai.user_id = 2 -- REPLACE 2 with a real user id
...
-- reason_codes / behavioral_signals / control_reason_codes are independent
-- evidence flags, never a ranking or a decision. HYBRID_REVIEW_REQUIRED
-- does not mean an item is misclassified — it means periodic review is
-- part of this Purpose's own definition. PERSONAL_* codes describe
-- capital concentration and data completeness, never urgency — long
-- ownership and slow DOM are not inherently negative for Personal items.
-- Both cohort objects may be small-sample or entirely absent for a given
-- item; `comparable_evidence_available` / `liquidity_cohort_match` /
-- `LOW_COMPARABLE_CONFIDENCE` / `NO_COMPARABLE_EVIDENCE` /
-- `PURPOSE_MATCHED_LIQUIDITY_COHORT_UNAVAILABLE` exist so a consumer can
-- see exactly how much evidence backs any given comparison, rather than
-- silently trusting a thin cohort.
